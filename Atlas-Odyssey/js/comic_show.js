...
function writePageTitle(div,toggleNum, char) {
  if (pgData.length >= pg) {
    ...
    document.querySelector(div).innerHTML = `<h3>${pgData[pg - 1].title}</h3>`;
    if (toggleNum) {
        //toggle whether you want to display the page number
        document.querySelector(div).innerHTML = `<h3>${pgData[pg - 1].pgNum + char + pgData[pg - 1].title}</h3>`; //char denotes a separating character between the number and the title
    }
  }
}
...
function writePage() {
  ...
  if (pgData.length < pg) { //if the array is blank or not long enough to have an entry for this page
    ...
    //
    page = `<img id="imgWide" alt="` + altText + `"title="` + altText + `" src="` + path + `" />`;
    return page;
    ...
    for (let i = 1; i < pgData[pg-1].imageFiles+1; i++) { //for loop to put all the parts of the image on the webpage
      ...
      page += `<img id="imgWide" alt="` + altText + `"title="` + altText + `" src="` + path + `" />`; //add page segment
      }
    } else {
      page = `<img id="imgWide" alt="` + altText + `"title="` + altText + `" src="` + path + `" />`;
    }
...
function writeNav(imageToggle) {
    ...
    function firstButton() {
        ...
        if (pg > 1) {
            ...
            return `<button class='button'><a href="?pg=` + 1 + navScrollTo + `"/>` + imgOrText(imageToggle, 0) + `</a></button>`;
        } else {
            if (!imageToggle) {
    ...
    function prevButton() {
        ...
        if (pg > 1) {
            //wait until page 2 to make button active
            return `<button class='button'><a href="?pg=` + (pg - 1) + navScrollTo + `"/>` + imgOrText(imageToggle, 1) + `</a></button>`;
        } else {
            if (!imageToggle) {
    ...
    function nextButton() {
        ...
        if (pg < maxpg) {
            ...
            return `<button class='button'><a href="?pg=` + (pg + 1) + navScrollTo + `"/>` + imgOrText(imageToggle, 2) + `</a></button>`;
        } else {
            if (!imageToggle) {
    ...
    function lastButton() {
        ...
        if (pg < maxpg) {
            ...
            return `<button class='button'><a href="?pg=` + maxpg + navScrollTo + `"/>` + imgOrText(imageToggle, 3) + `</a></button>`;
        } else {
            if (!imageToggle) {
